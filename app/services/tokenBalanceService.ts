...
export class TokenBalanceService {
  ...
  private async initializeTokenList() {
    try {
      ...
      tokens.forEach((token: { mint: string; symbol?: string; name?: string; decimals?: number; logoURI?: string }) => {
        // Only add tokens that have required fields
        if (token.symbol && token.name && token.decimals !== undefined) {
          this.tokenList.set(token.mint, {
            symbol: token.symbol,
            name: token.name,
