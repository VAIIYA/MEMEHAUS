...
import { uploadTokenImage as uploadToLighthouse } from './lighthouse';
...
export class CreateTokenService {
  ...
  /**
   * Prepare metadata for the token using MongoDB-backed API and Lighthouse for images
   */
  private async prepareMetadata(params: TokenCreationParams, creatorWallet: string, mintAddress: string): Promise<{ metadataUri: string; imageUrl: string }> {
    try {
      console.log('Preparing metadata with MongoDB-backed API...');

      // 1. Handle image upload via Lighthouse
      let imageUrl = params.imageUrl || `https://gateway.lighthouse.storage/ipfs/QmZ8v9gYndzB5zD54fG2D8yC8uGzY1X9XyvQZ2W9A9B9C9`; // Placeholder

      if (params.imageFile) {
        console.log('Uploading token image to Lighthouse...');
        try {
          imageUrl = await uploadToLighthouse(params.imageFile, params.symbol);
          console.log(`✅ Token image uploaded to Lighthouse: ${imageUrl}`);
        } catch (imageError) {
          console.warn('❌ Image upload to Lighthouse failed, using placeholder:', imageError);
        }
      }

      // 2. Set the metadata URI to point to our own API
      // This API will fetch the token data from MongoDB and serve Metaplex-compliant JSON
      const domain = typeof window !== 'undefined' ? window.location.origin : 'https://memehaus.vercel.app';
      const metadataUri = `${domain}/api/token/${mintAddress}/metadata`;

      console.log(`✅ Metadata URI set to: ${metadataUri}`);
      ...
      console.error('Error in prepareMetadata:', error);
      // Absolute fallback if everything fails
      return {
        metadataUri: `https://memehaus.vercel.app/api/token/${mintAddress}/metadata`,
  ...
  async createToken(
    ...
    try {
      ...
      // Prepare metadata and upload image to Lighthouse
      console.log('Preparing metadata and uploading image...');
      const { metadataUri: metadataUriResult, imageUrl: uploadedImageUrl } = await this.prepareMetadata(params, wallet.publicKey.toBase58(), mint.publicKey.toBase58());
      ...
      // Store token data in MongoDB via API route
      console.log('Storing token data in MongoDB...');
      try {
        await this.storeTokenData({
          mintAddress: mintAccount.toBase58(),
          tokenAccount: creatorTokenAccount.toBase58(),
        ...
        console.warn('⚠️ Non-critical: Failed to store token data in MongoDB:', storageError);
      }
      ...
      return {
        ...
        metadataSignature: mintSignature, // Metadata is part of mint transaction
      };
  ...
  /**
   * Store token creation data in MongoDB
   */
  private async storeTokenData(tokenData: {
    mintAddress: string;
    tokenAccount: string;
    ...
    try {
      ...
      // Return result
      return {
        success: storageResults.mongodb.success,
        url: undefined,
        error: storageResults.mongodb.success ? undefined : `MongoDB: ${storageResults.mongodb.error}`,
      };
    } catch (error) {
